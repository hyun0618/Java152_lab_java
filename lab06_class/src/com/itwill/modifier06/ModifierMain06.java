...
/*
 * 접근 수식어(access modifier)는 지역 변수에는 사용할 수 없음! => private int x = 1; 불가능
 * 
 * final 수식어 
 ...
 *   (4) 정적 필드는 JRE가 사용하는 메모리 영역 중엑 메서드 영역에 생성.
...
//		message = "a";  message 는 스태틱이 아닌 필드이므로 실행할 수 없음. 
...
//		n2 = 100;  final 지역변수의 값은 변경(재할당)할 수 없음. 
...
//		n3 = 3;  => 변경 불가능
		...
		ModifierMain06 app = new ModifierMain06("버스 파업");  // 참조타입(4바이트) => 스택에 4바이트로 주소 저장 
//		app.message = "";  final 필드는 객체 생성 이후에 값을 변경할 수 없음. 
		
		// Test 클래스의 스태틱 멤버 사용: 
		System.out.println("Test.y = " + Test.y);
		Test.y = 100; // 스태틱 필드는 객체 생성과 관계없이 사용할 수 있음.
		System.out.println("Test.y = " + Test.y);
		Test.printFields2();
		
		// Test 클래스의 인스턴스 멤버 사용: 먼저 객체를 생성해야 함.
		Test test = new Test();
