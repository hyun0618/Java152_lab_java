...
public class FileMain02 {
	...
	public static void main(String[] args) throws Exception {
		...
		String orgin = "data/ratings.dat"; // 원본 파일
		...
		while (true) {
			// (1-1) read() 메서드 사용: 
			// int b = in.read(); // 파일에서 1바이트를 읽어서 4바이트에 저장.
			
			// (2-1) read(byte[] b) 메서드 사용: 
			byte[] buffer = new byte[4* 1024]; // 4,096바이트(4KB) 크기의 빈 배열 생성
			int b = in.read(buffer); // read한테 0으로만 채워진 배열을 넘겨준다.
			// => buffer: 파일에서 읽은 내용을 저장하기 위한 바이트 배열.
			// => 리턴 값: 파일에서 실제로 읽은 바이트 수(int).
			
			if (b == -1) { // EOF				
				break; // 무한루프 종료
			}
			// (1-2) write(int b) 메서드 사용:			
			// out.write(b); // 파일에 1바이트를 씀.	
			
			// (2-2) write(byte[] b) 메서드 사용:
			// out.write(buffer); // 바이트 배열의 내용을 그대로 파일에 씀. 4KB를 파일에 씀.
			
			// (3-2) write(byte[] buf, int off, int len) 메서드 사용: 
			out.write(buffer, 0, b);
			// => byte[] buf: 파일에 쓸 데이터를 가지고 있는 바이트 배열.
			// => int off: 배열 b에서 읽기 시작할 인덱스. offset.(건너뛰는 위치)
			// => int len: 배열에서 실제로 파일에 쓸 바이트 길이.  
		}
