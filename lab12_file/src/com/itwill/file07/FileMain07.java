...
/*
 * IO Stream: 
 *   - 바이트 스트림(byte stream). 읽기/쓰기의 기본 단위는 byte 단위.
 *   - 모든 종류의 파일(txt, jpg, mp4, pptx, pdf, ...)에서 사용이 가능.
 * 
 * InputStream
 ...
 * 
 * Reader/Writer: 
 *   - 문자 스트림(character stream). 읽기/쓰기의 기본 단위는 "문자(character)".
 *   - 텍스트 파일(txt, csv, ...)에서만 사용이 가능.
 * 
 * Reader
...
public class FileMain07 {
	...
	public static void main(String[] args) throws Exception {
		...
		// 3. 읽기 속도를 빠르게 하기 위해서 버퍼(메모리)를 사용. => 옵션
		BufferedReader reader = new BufferedReader(isr);
		
		// 무한 반복문
		while (true) {
			int read = reader.read(); // 파일에서 한 글자씩 읽음.
			if (read == -1) { // EOF
				break;
			}
